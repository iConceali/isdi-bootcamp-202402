// api/services/tests/cryptoData/fetchCryptoData.spec.js
...
import fetchCryptoData from "../../cryptoData/fetchCryptoData.js";
...
describe("fetchCryptoData", () => {
  ...
  it("fails due to validation error in API response", async () => {
    ...
    try {
      ...
      throw new Error("Expected fetchCryptoData to throw a validation error");
    } catch (error) {
  ...
  it("fails due to API error", async () => {
    ...
    try {
      await fetchCryptoData();
      throw new Error("Expected fetchCryptoData to throw an API error");
    } catch (error) {
      expect(error).to.be.instanceOf(SystemError);
      expect(error.message).to.include("Failed to fetch crypto prices");
    }
