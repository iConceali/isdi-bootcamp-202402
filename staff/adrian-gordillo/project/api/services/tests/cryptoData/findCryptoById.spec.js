...
import findCryptoById from "../../cryptoData/findCryptoById.js";
...
describe("findCryptoById", () => {
  ...
  it("succeeds in finding a crypto by ID", async () => {
    const foundCrypto = await findCryptoById(crypto.id.toString());

    expect(foundCrypto).to.exist;
    expect(foundCrypto.id.toString()).to.equal(crypto.id.toString());
