...
describe("updateDeposit", () => {
  ...
  it("succeeds on updating the user's deposit", async () => {
    ...
    const result = await updateDeposit(user._id.toString(), newDeposit);

    expect(result).to.have.property("deposit", newDeposit);
  ...
  it("fails on invalid deposit format", async () => {
    ...
    try {
      ...
      await updateDeposit(user._id.toString(), invalidDeposit);
    } catch (error) {
      expect(error).to.be.instanceOf(ContentError);
      expect(error.message).to.include(
        `deposit ${invalidDeposit} is not a valid number`
