//api/services/arbitrageTriangularDetector.js

import axios from "axios";

const getSymbolPrices = async (symbol) => {
  try {
...
export const detectArbitrage = async () => {
  const triangles = [
  ...
  for (const triangle of triangles) {
    const tickers = await Promise.all(triangle.map(getSymbolPrices));
