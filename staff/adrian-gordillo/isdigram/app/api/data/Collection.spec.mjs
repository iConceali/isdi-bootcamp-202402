  ...
  describe("> CRUD", () => {
    ...
    describe("updateOne", () => {
      it("should update an existing document", (done) => {
        fs.writeFile(
          "./data/cars.json",
          '[{"brand":"porsche","model":"911"},{"brand":"fiat","model":"500", "id": "2"}]',
          (error) => {
            if (error) {
              console.error(error);
              return;
            ...
            const updatedDocument = JSON.parse(
              '{"brand":"renault","model":"megane", "id": "2"}'
            );

            cars.updateOne(updatedDocument, (error) => {
              if (error) {
                console.error(error);
                return;
              }
              ...
              cars._loadDocuments((error, documents) => {
                ...
                expect(documents[1].brand).to.equal("renault");
                expect(documents[1].model).to.equal("megane");
